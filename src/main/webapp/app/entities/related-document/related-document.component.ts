...
@Component({
    selector: 'jhi-related-document',
    templateUrl: './related-document.component.html'
})
export class RelatedDocumentComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.relatedDocumentService.search({
                page: this.page - 1,
                query: this.currentSearch,
                size: this.itemsPerPage,
                sort: this.sort()}).subscribe(
                    (res: HttpResponse<RelatedDocument[]>) => this.onSuccess(res.body, res.headers),
                    (res: HttpErrorResponse) => this.onError(res.message)
                );
            return;
        }
        this.relatedDocumentService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
                (res: HttpResponse<RelatedDocument[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/related-document'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/related-document', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
